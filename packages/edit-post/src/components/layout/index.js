...
function MetaBoxesMain() {
	...
	const [ setResizable, bindResizableHandle ] = useResizableBox( {
		...
		onResizeStart: ( { size: [ , ySize ] } ) => {
			// Pointer input won’t change the size but keyboard input will.
			applyHeight( ySize );
		},
		onResize: ( { size: [ , ySize ] } ) => {
			applyHeight( ySize );
	...
	if ( isShort ) {
		...
		paneProps = {
			...
			style: { height: openHeight, maxHeight: usedMax, minHeight: min },
		};
		paneButtonProps = {
			ref: separatorRef,
			role: 'separator',
